  ...
  req.on('end', () => {
    ...
    if (url.startsWith('/storage/v1/object/list/')) {
      ...
      return json(
        res,
        200,
        names.map((k) => ({ name: k.slice(bucket.length + 1), id: k })),
      );
    }
  ...
  return new Promise((resolve) => {
    server.listen(0, '127.0.0.1', async () => {
      ...
      try {
        ...
        console.log('\n[1] import-content.js against mock Supabase');
        await run('node', [path.join(ROOT, 'scripts', 'import-content.js')], {
          env: { ...process.env, SUPABASE_URL: url, SUPABASE_SERVICE_ROLE_KEY: 'mock-service-key' },
        });
        ...
        // now points at the DB-native fixture post/event.
        ...
        console.log('\n[2] build.js against mock Supabase');
        ...
        check('admin bundle has anon URL injected', adminJs.includes(url));
        check('admin bundle does NOT contain the service key', !adminJs.includes('mock-service-key'));
        check('admin page does NOT contain the service key', !adminHtml.includes('mock-service-key'));
        const allPublic = [];
        const walk = (d) => {
        ...
        const leaked = allPublic.filter((p) => {
          try {
            return fs.readFileSync(p, 'utf8').includes('mock-service-key');
          } catch {
            return false; // binary
          }
        });
        check('service key appears in NO file under public/', leaked.length === 0);
